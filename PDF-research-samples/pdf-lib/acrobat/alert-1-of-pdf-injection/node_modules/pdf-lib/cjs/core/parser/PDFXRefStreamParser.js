    ...
    function PDFXRefStreamParser(rawStream) {
        ...
        if (Index instanceof PDFArray_1.default) {
            ...
            for (var idx = 0, len = Index.size(); idx < len; idx += 2) {
                var firstObjectNumber = Index.lookup(idx + 0, PDFNumber_1.default).value();
                var length_1 = Index.lookup(idx + 1, PDFNumber_1.default).value();
        ...
        for (var idx = 0, len = W.size(); idx < len; idx++) {
            this.byteWidths[idx] = W.lookup(idx, PDFNumber_1.default).value();
        }
    }
